...
def get_value(prop_name: str):
    """
    Get property value from environment variables or default value
    ...
    if isinstance(default_value, str):
        ...
        return os.environ[evn_var_name] == "true" if evn_var_name in os.environ else default_value
    elif isinstance(default_value, int):
        return int(os.environ[evn_var_name]) if evn_var_name in os.environ else default_value
    elif isinstance(default_value, float):
        return float(os.environ[evn_var_name]) if evn_var_name in os.environ else default_value
    else:
        raise Exception(f"Unknown type for property: {prop_name}. Type: {type(default_value)}")
...
def build_mat(day: str):
    ...
    objects = s3.list_objects_v2(Bucket=get_bucket(), Prefix=f"{get_value("input-folder")}/day={day}/")
    for key in [key["Key"] for key in objects.get("Contents", [])]:
        ...
        with gzip.open(tmp_file.name, "rt") as f:
            for idx, line in enumerate(f):
                ...
                row_arr.append(add_key(int(line_data[0]) if convert_key_to_int else line_data[0]))
                col_arr.append(add_key(int(line_data[1]) if convert_key_to_int else line_data[1]))
    ...
    mat = csr_matrix((distances, (row_arr, col_arr)), shape=(len(key_to_index), len(key_to_index)))
...
def cluster_data(mat):
    ...
    if mat is not None and mat.shape[0] > 0 and mat.shape[1] > 0:
        model = DBSCAN(eps=get_value("epsilon"),
                       min_samples=get_value("min-samples"), metric='precomputed')
